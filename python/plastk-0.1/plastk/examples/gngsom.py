...
This example creates a simple environment and agent to illustrate how
to train PLASTK vector quantizers. The environment generates
sensations in the form of two dimensional vectors drawn from a
constantly changing Gaussian distribution.  The agent accepts each
sensation and passes it to two vector quantizers, a Kohonen
self-organizing map (SOM) and a growing neural gas (GNG).  The agent
produces no actions.  A simple GUI allows the user to visualize the
SOM and GNG as they change over time.

$Id: gngsom.py 229 2008-03-03 04:38:00Z jprovost $
"""
...
from plastk.base.rl import Agent,Environment
...
EquilibriumGNG.lambda_ = 50
EquilibriumGNG.max_age = 20
EquilibriumGNG.e_b = 0.05
EquilibriumGNG.e_n = 0.001
EquilibriumGNG.print_level = VERBOSE
