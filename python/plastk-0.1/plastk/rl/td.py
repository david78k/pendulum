...
Reinforcement learning agents that learn by methods of temporal differences.

$Id: td.py 229 2008-03-03 04:38:00Z jprovost $
"""

from plastk.base.params import Parameter,Number,Magnitude,Integer
from plastk.base.rl import Agent,is_terminal
from plastk.misc.utils import mmax,weighted_sample,inf
...
import numpy
from numpy import nonzero,putmask,array,zeros,dot,argmax
...
class TDAgent(Agent):
    ...
    def learning_step(self,training_fn,sensation,reward=None):
        ...
        if self.allow_learning:
            training_fn(self.last_sensation,self.last_action,reward,sensation,next_action)
    ...
    def _sarsa_training(self,sensation,action,reward,next_sensation,next_action):
        """
        ...
        self.verbose(("Q(t-1) = %.5f, Q(t) = %.5f, diff = %.5f,"+
                      ...
                      % (last_value,value,value-last_value,
                         delta,is_terminal(next_sensation)))        

        self.update_Q(sensation,action,delta)


    def _q_learning_training(self,sensation,action,reward,next_sensation,next_action=None):
        """
...
class TabularMemoryTDAgent(TabularTDAgent):
    ...
    """
    memory_steps = Integer(default=1,bounds=(0,None))
...
def softmax(ar,temp):
    ...
    if temp == 0 or inf in ar:
        ...
        numer = numpy.exp(ar/float(temp))
        denom = numpy.sum(numer)    
        return numer/denom
