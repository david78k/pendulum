...
An environment for swinging up or balancing the inverted pendulum.
The environmmaintains the pendulum's state as angle and angular
velocity and updates it according to the physics of a simple pendulum.
The agen'ts actions are torques that can be applied.

$Id: pendulum.py 229 2008-03-03 04:38:00Z jprovost $
"""
import plastk.base.rl as rl
...
class Pendulum(rl.Environment):

    initial_angle = params.Number(default=0)
    initial_velocity = params.Number(default=0)
    delta_t = params.Number(default=0.01)

    mass = params.Number(default=1.0,bounds=(0,None))   # mass in kg
    length = params.Number(default=1.0,bounds=(0,None))  # length in meters

    g = params.Number(default=9.8)  # gravitational acceleration in m/s^2

    friction = params.Number(default=0.01,bounds=(0,None))
    
    actions = params.Parameter([-1,0,1])   # actions are torques in newton-meters

    reward_exponent = params.Number(default=1)
    reward_velocity_threshold = params.Number(default=0.25)
...
class PendulumUpright(PendulumEpisodic):
    initial_angle = params.Dynamic(default=lambda:rand.normal(0,5*pi/180))
    def reward(self):
        return 1
    def is_terminal(self):
...
def test(n=1000,gui=True):
    from rl.loggingrli import LoggingRLI
    ...
    agent = rl.UniformTiledSarsa(actions = p.actions,
                                 num_features = 1024,                                        
                                 num_rfs = 1,
                                 num_tilings = 1,
                                 tile_width = 2*pi/16,
...
def torquetest(pendulum,torque):
    ...
    rli = rl.LoggingRLI(name = "Torque Test, torque = "+`torque`)
