...
$Id: options.py 229 2008-03-03 04:38:00Z jprovost $
"""

import plastk.base.rl as rl
from plastk.misc.utils import Stack
...
class GeneratorOption(Option):
    fn =      params.Callable(default=None)
    args =    params.List(default=[])
    appl_fn = params.Callable(default=None)
...
class GeneratorMethodOption(GeneratorOption):
    obj =         params.Parameter(default=None)
    method =      params.String(default=None)
    appl_method = params.String(default=None)
...
class Primitive(Macro):
    action = params.Parameter(default=None)
    def __init__(self,**args):
...
class OptionAdapter(Option):
    agent = params.ClassSelector(rl.Agent,default = None)
    def __init__(self,**args):
...
class HierarchicalAgent(rl.Agent):
    ...
    root_option = params.ClassSelector(Option,default=None)
