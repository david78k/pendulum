...
A similar environment to that in rl.gridworld, but the agent's pose or
'facing direction' is maintained as part of its state.  The agent's
actions are to turn right or left and travel forward (and backward?).

$Id: facegridworld.py 229 2008-03-03 04:38:00Z jprovost $
"""
...
class FaceGridWorld(GridWorld):
    """
    FaceGridWorld is a dummy wrapper around GridWorld, which adds a
     (random) curr_face attribute to model the direction the agent is
     facing.  FaceGridWorld is used for testing, but with different
     actions, could be a simple Turn,Travel Sim
    """
    turns = []
    directions = ['N','S','E','W']
...
class MarkovLocPOMDPWorld(rl.Environment):
    ...
    POMDP_MarkovLoc_Antie models a corridors and intersections
    gridworld with discrete actions and complex, symbolic observations
    (views down the hallways).  Views include observations of objects
    in the environment, such as furniture, flooring appearance, and
    wall paintings.
