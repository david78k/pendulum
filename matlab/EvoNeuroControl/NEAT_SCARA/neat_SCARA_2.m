...
%[xgArray ygArray zgArray]         = MakeGoalArray(100);
%save('goalsave.txt','-ascii','xgArray','ygArray','zgArray')
...
%parameters of main file
    maxgeneration=1001; %maximum number of generations for generational loop    
  ...
  %parameters initial population
    ...
    %inputs:
    % theta_1 para el angulo del primer link
    % theta_2 para el angulo del segundo link
    % theta_3 para el angulo del tercer link
    % xg coordenada x del objetivo
    % yg coordenada y del objetivo
    number_input_nodes=2; 
    
    %outputs
    % theta_1 coordenada de salida para link 1
    % theta_2 coordenada de salida para link 2
    % theta_3 coordenada de salida para link 3
  ...
  %reproduction parameters
    %stagnation+refocuse
    stagnation.threshold=1e-3; %threshold to judge if a species is in stagnation (max fitness of species varies below threshold) this threshold is of course dependent on your fitness function, if you have a fitness function which has a large spread, you might want to increase this threshold 
    ...
    stagnation.number_generation=inf; 
    
    
    %if maximum overall fitness of population doesn't change within
    %threhold for this number of generations, only the top two species are allowed to reproduce                                
    ...
    % species which have equal or greater than number_copy individuals will have their best individual copied unchanged into the next generation                           
    initial.number_copy=2; 
    
    %selection (ranking and stochastic universal sampling)
    selection.pressure = 2.0; % Number between 1.1 and 2.0, determines selective pressure towards most fit individual of species
    ...
    %randomly from both parents. In the
    %(1-crossover.probability_multipoint) cases, weights of the new connection genes are the mean of the corresponding parent genes
    crossover.probability_multipoint=0.6; 
    

    
    %mutation
    mutation.probability_add_node=0.002;
    mutation.probability_add_connection=0.002;
   
    % no le pongamos recurrencia por ahora pues la tarea es directa y no
    % hay historia
    ...
    mutation.probability_mutate_weight = 0.5;
    mutation.weight_cap   = 1000; % weights will be restricted from -mutation.weight_cap to mutation.weight_cap
    mutation.weight_range = 1; % random distribution with width mutation.weight_range, centered on 0. mutation range of 5 will give random distribution from -2.5 to 2.5
...
while generation<maxgeneration && flag_solution==0
   ...
   if save_flag==1 % Backup copies of current generation
      ...
      %save('neat_scara_mse.txt','-ascii','mseArray');
   end
      
   % call evaluation function (in this case the performance of the planar robot),
   %fitnesses of individuals will be stored in population(:).fitness
   %IMPORTANT reproduction assumes an (all positive!) evaluation function where
   %a higher value means better fitness (in other words, the algorithm is geared
   %towards maximizing a fitness function which can only assume values between 0 and +Inf) 
   
   [population  mse] = Experiment(population);
  
   %population=fulladder_experiment(population);
   
   %display([' Generation ',num2str(generation)])
   ...
   if maximale_fitness>1000000

      flag_solution=1;
   end
   
   display([' Average Non Disabled Connections: ',num2str(a)])   
   display([' Average Hidden Nodes: ',num2str(b)])   
   subplot(2,2,1);
...
%      ylabel('fitness');
%      minmse = min(mse);
%      mseArray(generation)=minmse;         
%      plot(0:generation-1,mseArray,'k');    
%      disp(['Iteration: ',int2str(generation),' Best Fitness ',num2str(minmse)])
%      title([' Mean Squared Error: ',num2str(minmse)])
%      ylabel('mse');
%      %xlabel('generation');
%      drawnow;
%      hold on
