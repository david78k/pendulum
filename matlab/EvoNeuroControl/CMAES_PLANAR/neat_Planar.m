...
%[T1Array T2Array T3Array] = MakeTArray(100);
%[xgArray ygArray]         = MakeGoalArray(100);
%save 'goalsave' xgArray ygArray
load  goalsave % load the information of the goals in order to train always with the same goal set
  %parameters of main file
    maxgeneration=20000; %maximum number of generations for generational loop    
...
while generation<maxgeneration && flag_solution==0
   ...
   [population  mse] = Arm_Experiment(population);
...
%    drawnow; 
     ...
     %plot(max_overall_fitness(2,:),max_overall_fitness(1,:));
     %ylabel('fitness');
     mseArray(generation)=min(mse);     
     plot(0:generation-1,mseArray,'k');     
     title([' Mean Squared Error: ',num2str(min(mse))])
     ylabel('mse');
     %xlabel('generation');
     drawnow;
     hold on
