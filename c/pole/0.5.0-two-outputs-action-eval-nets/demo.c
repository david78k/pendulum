  ...
  while (i < num_trials && j < TARGET_STEPS) /* one hour at .02s per step */
    {
      Cycle(1);
      if (DEBUG == 2 && j % 1000 == 0)
        printf("Episode %d step %d rhat %.4f %.4f\n", i, j, r_hat[0], r_hat[1]);
      j++;
...
void updateweights() {
  ...
  double factor1, factor2;
	...
	{
	  //for(j = 0; j < 2; j++) {
	  //factor1 = Beta_h * y_old[i] * (1.0 - y_old[i]) * sgn(c[i]);
	  //factor2 = Rho_h * z[i] * (1.0 - z[i]) * sgn(f[i]) *
	  factor1 = Beta_h * r_hat[1] * y_old[i] * (1.0 - y_old[i]) * sgn(c[i]);
	  factor2 = Rho_h * r_hat[1] * z[i] * (1.0 - z[i]) * sgn(f[i]) *
